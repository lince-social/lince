...
Lince works with a PostgreSQL database, some answers about data types can be found there, lince searches for it's data in .config/lince/lince.sql, if not found there, it defaults to the directory lince was cloned to, in src/db/versions/lince.sql. It is recomended to frequently backup the lince.sql file, if some error or mistake happens, your information is safe.
