...
This branch began as **Lingua**, and Lingua began as one question: how can two
people who have never met use the same word and be sure they mean the same
thing. It turned out to be the smaller half of a larger question. Before you
can share a meaning you have to be able to hold one, and holding one is the
whole problem — what a thing is, what has been said about it, who said it, and
what any of that is worth to the person reading it later.

So Ontology stopped being a vocabulary and became the floor. It is the small
set of shapes everything above is made from, and the reason Lince does not grow
a new table every time it grows a feature. Four shapes carry nearly all of it:
...
moving them between Organs. A Transfer is a promise about them; a Karma rule is
arithmetic over them. When something new is proposed here the first question is
which of the four it already is — and a genuinely fifth shape has not been
needed yet.

The parts below take one shape each, with the room to explain themselves.

## Lince today, in one read

Everything above is the model and the record of how each piece was decided.
This is the short version: what Lince is, how it works, and what it is for.
It exists so the task list below can be read without holding six hundred
lines of reasoning in your head.
...
you fully trust them, and a mailbox door for leaving sealed batches with a
carrier when the recipient is switched off.
...
**This is NOT planned work.** No boxes, deliberately — it is
...
- [x] **`log_local_tx` never resolves `replica_root`** — FIXED 2026-08-16, and
  the leak was wider than this entry said. Pre-existing, found during the C1
  audit and NOT introduced by it: the transaction-path logger wrote
  `replica_root = NULL` while its pool twin resolved it and branched the outbox
  on it, so every write taking the transaction path — assertions above all,
  which is what a conversation is made of — was stamped general-feed and queued
  to every `sync_out` contact regardless of the replica it sat in. It now calls
  `replica::root_for_op_tx`, so the two loggers answer identically. Landed
  ahead of C5 rather than inside it: whether a private write stays private is
  not a scoping feature.
...
**Running `iroh-relay` on the VPS is a C12 box now** (moved 2026-08-18), along
with every other item in this section that needs a second machine. It needs a
public IP, a DNS name, and TLS (the relay speaks HTTPS/WebSocket to nodes);
point the Cells at it as their configured relay instead of the default public
ones. From then on the Organ's own machine carries its own connection metadata.
The reference for what to build stays here, because it is what the Resenha
scenario has to reproduce — only the checkbox moved.
...
**Running a Cell on the VPS as a member of the Organ roster** — the always-on
device that makes offline delivery work without either laptop being up — and
the conclusion that follows from it, **that relay-only mode then costs nothing
that matters** because the relay being depended on is your own, are both C12
boxes (moved 2026-08-18). Depending on your own machine is not a dependency
problem.
...
- [x] **Both relay jobs on one VPS, as separate everything** — built
  2026-08-15. Separate processes, ports, system users and state directories,
  with no interaction and no shared keys; neither may read the personal Cell's
  store, which is why the personal Cell belongs on a different machine
  entirely. `scripts/deploy/nixos/vps-module.nix` imports both, and
  `nixosModules.vps` / `nixosModules.iroh-relay` are exported from the flake —
  the relay module had sat on disk for two days UNEXPORTED, so the deployment
  described here could not be assembled by anyone following it. Four blurrings
  now refuse to deploy rather than merely being discouraged: a shared user, a
  shared group, a port collision, and a shared state directory.
...
**Both shapes are C12 boxes** (moved 2026-08-18); the description of each stays
here because it is what the scenario has to reproduce.
...
`mode = "server"`, and it is the one the always-on-Cell text above describes.

**A blind mailbox** — holds SEALED envelopes it cannot read and hands them over
when the recipient appears. It is not a member of any roster, holds no keys,
and converges nothing; it stores and forwards. This is `mode = "relay"`, and it
is the one case where encrypting op payloads earns its cost (see "Op
authenticity"), precisely because a third party is carrying the bytes.
...
- [x] **Organ sand controls the whole pairing per contact** — built. The
  Record sync selector writes `organ_sync_policy` as none / outgoing (my
  records go there) / incoming (their records land here) / both, over the
  existing `sync_out`/`sync_in` flags, defaulting to none. **Live-only was not
  built as a fourth option** and is not missing by oversight: the live door is
  a separate door, not a degree of record sync, and folding it into this
  selector would make one control mean two things. The `mode` half of this
  entry is still open — see "Branch on the `mode` column" below.
- [x] **Arbitrary configurable Protein filter for File Sync selection** —
  built 2026-08-14, see the note at the end of this section. `configured_filter`
  reads one `Predicate` from `lince.file_sync.filter` and ANDs it with the
  Organ origin; an unparseable filter is IGNORED rather than failing closed,
  with the surface saying so. **`file_sync.rs`'s own module doc still says
  "Selection is deliberately NOT Protein-configurable yet"** — stale since
  2026-08-14 and worth deleting, because it is the only thing in the tree that
  still claims this is undone.
...
The release-train half of this decision is **deferred to the end of this
document** — see "The release train, when there is anyone to plan around".
`base_url` and the `Introduction` change did not wait for it: they broke the
wire on 2026-08-19 and that was correct, because nobody is running Lince to be
cut off.
...
- [x] **A `snapshot` op kind** — built 2026-08-10, the enabling half of the log
  work. Identity minted from the producing Cell and its HLC like any other op.
  This reverses "op kinds, written down as a closed set, with snapshots
  explicitly NOT among them" — that exclusion was correct while snapshots were
  local and is wrong once they must travel.
- [x] **`crdt` ops below a snapshot become prunable** — built 2026-08-10, which
  is what finally bounds the one part of the log that grows with edit history
  rather than with live state. A `crdt` op is prunable only once a `snapshot`
  sits above its record's tail and the normal checkpoint-gated retention
  agrees. Extension ops are deliberately outside this: they have no snapshot to
  become prunable under.
...
- [x] **A stated total budget per Cell** — built 2026-08-18. `store::budget`
  "holds one ceiling on the singleton configuration row" (2 GiB default, `0`
  meaning unlimited) and divides it into fixed shares: media 70, Facade 25,
  quarantine 5. **Only the total is configurable, and that is the design.** The
  owner's decision is how much of their disk Lince may use; how Lince divides it
  internally is Lince's job, and three sliders nobody understands turn a budget
  into an unanswerable question instead of an answer to one. Shares are integer
  percentages summing to 100, pinned by a test — a set that quietly summed to 90
  would waste a tenth of the disk and nothing would report it.
  **This Cell's number, never the Organ's.** It lives in `configuration` rather
  than in the Ledger, so it does not sync: a phone and a VPS have no reason to
  agree, and syncing it would make the smallest device's ceiling everybody's.
- [x] **The budget is visible and answerable** — built 2026-08-18. The
  Configuration sand's Storage page reads `GET /host/storage` and shows each
  area's used-of-allowed with a bar, plus the honest total. A host route rather
  than a Protein subscription because none of it is a Record: it is bytes on
  this machine.
  **The total includes what is never evicted.** The database and published DNA
  packages are reported as `unbudgeted` and left alone — they are the owner's
  own work rather than a cache of somebody else's, and evicting them would be
  deleting rather than reclaiming. A reported total that does not match what the
  owner's file manager says is how a budget surface loses their trust the first
  time they check it.
...
## How the work is taken

The open work is the task list at the top of this file, plus the two Records
still carrying work of their own:
[[Reachability and identity|r_ZSG0K8Y1XSV5T0TMZHVMG3VPC9]] and
[[C5 Scoping|r_K3XKXDKT1EQA7ZDH7F79KNP32S]]. State is read from a Record's
quantity; a box here and a box in the Record would be the same claim written
twice.

**Work closes behind you, not ahead of you** (the working rule, also in
...
- Expand a task freely when completing it honestly demands work nobody listed.
  This list is a plan, not a contract.
...
- **A box is not done until a HUMAN CAN USE IT.** Every box that adds a
  ...
  all. Split the surface out only when it genuinely belongs elsewhere, and then
  say so in both places. Two standing riders: "obvious from the API" is not a
  surface, and a panel showing nothing must say WHICH nothing it means
  ("none yet" / "not switched on" / "cannot reach anyone"), because the empty
  state reads as a broken feature otherwise. Reachability accrued this debt
  across enrolment, the front door and the directory record, and paid it.

### What the finished work turned out to be

"**Landed work is not a Record any more**" (2026-08-18). The convention this
folder runs on is that when work lands its text folds into the prose of the
thing it belonged to and the task Record is deleted — so an idea's description
thickens with what was actually built, and what is still a Record is always
exactly what is left. Seven of these folded in here. Their reasoning is kept
verbatim rather than summarised: most of it is why a decision went the way it
did, which is the half a later reader cannot reconstruct.

### Bedrock

**Bedrock. Complete 2026-08-11.** Quarantine's two follow-ons
(rate-limiting, the sand view) moved to C5 rather than sitting under a ticked
this work — neither is bedrock.
...
### The Organ and Cell split

**The Organ/Cell split. Built 2026-08-10.** `actor_organ` became
...
**A complete log. Enabling half built 2026-08-10.** Snapshot op
...
**the repair half. Complete 2026-08-13.**
...
per-contact "Check we agree", so it is now closed on both halves:
detect locally, detect against a peer, repair locally.

### Storage budgets

**Storage budgets (decision 3). Complete 2026-08-18.** Snapshot-plus-truncate
was always "most of the answer" and landed with the log work — the op log is
O(live state) for text as it already was for scalars. What this added is
the other part: a stated ceiling, divided into shares, each area evicting inside
its own.

**One number the owner sets, and shares Lince decides.**
`configuration.storage_budget_bytes` is a per-Cell total, 2 GiB by default, `0`
meaning unlimited — and unlimited had to be expressible, because the point of a
budget is that the owner decides and "no ceiling" is one of the decisions. It
divides media 70, Facade 25, quarantine 5, as integer percentages summing to 100
and pinned by a test. Only the total is configurable: how much of their disk
Lince may use is the owner's call, how Lince divides it internally is Lince's,
and three sliders nobody understands turn a budget into an unanswerable question
instead of an answer to one.

**Per-area shares, because a single cap does not allocate — it races.**
Whichever area grows fastest evicts everything else, so importing photos would
silently empty the quarantine ring and one large Facade fetch would push out the
media a Record's body points at. With a share each, an area filling up hurts
that area and nothing else, which is the only version of this an owner can
reason about.

**The Cell's number, never the Organ's.** It lives on the configuration row
rather than in the Ledger, so it does not sync. A phone and a VPS have no reason
to agree, and syncing it would make the smallest device's ceiling everybody's.

**The per-source quota is one pure function, shared.**
`store::budget::evict_plan` takes entries newest-first and a quota and says what
to drop: the newest survives, an entry larger than the whole quota is dropped
rather than permanently occupying an area budgeted for many, and a corrupt
length evicts instead of wrapping negative into apparent free space. Quarantine
calls it per contact today, alongside its existing per-contact row count — the
two bounds answer different questions, "how much disk may one peer cost me"
versus "how much garbage from one peer is worth reading", and the tighter one
wins. Per contact rather than per Cell for the reason the count already was: a
global byte cap lets one peer bury the evidence of what a second peer did under
noise, which is exactly what somebody hiding an attack would do.

**The remainder is reported, never evicted.** The database and published DNA
packages are the owner's own work rather than a cache of somebody else's, so
they sit outside the ceiling and appear in the report as `unbudgeted`. A total
that does not match what the owner's file manager says is how a budget surface
loses their trust the first time they check it.

**Surface: the Configuration sand's Storage page.** `GET /host/storage` gives
each area's used-of-allowed with a bar and the honest on-disk total; `POST
/host/storage/budget` sets the ceiling in megabytes. Host routes rather than
Protein subscriptions because none of this is a Record — it is bytes on this
machine. The Facade row reads **"not built yet"** rather than `0 B`, because a
zero from an absent consumer must not look like a zero from an empty cache.

**What moved out, and where.** The Facade cache's own per-source quota is
**C9's box**: the mechanism above is written, tested and in use, and what is
missing is the cache itself, which C9 builds. It went to the work that owns
the missing consumer rather than to C12 — nothing about it needs a second Lince,
so filing it there would have made C12 a drawer for anything inconvenient
instead of the one thing it means.

The original deferral said building a budget for absent consumers would be
guessing. That was right about the Facade cache and wrong about the rest:
quarantine and the media store both exist, both hold other people's bytes, and
both were unbounded.

### Your own infrastructure

**Your own infrastructure. EMPTIED 2026-08-18, not completed.** The
"software half is built: discovery settings are ordinary config" (2026-08-13),
the Nix modules exist and both relay jobs coexist on one VPS with four
"blurrings refusing to deploy" (2026-08-15), and self-hosted address publishing
landed with C3's directory record. What was left — iroh-relay on the box, the
always-on Cell, `mode = "server"`, `mode = "relay"`, and the relay-only
conclusion that follows from them — is five DEPLOYMENTS, not five features,
and all five moved to C12.
**It is ticked because nothing implementable remains in it**, which
is a different claim from "your own infrastructure works". Say the second
only after C12.
**Surface:** the Discovery panel gains "where this Cell is reachable from" —
which relay, whose, and a plain statement of what relay-only mode costs. That
line is now C12's to satisfy, since it can only be true of a relay that is
actually running. A Nix module with no way to see whether it is working is a
config file, not a feature.

### Threads and references

**Threads and references. Complete 2026-08-14.** Live references
...
"**The map that used to live here is gone" (2026-08-19)**, along with the
grouping it mapped. Open work is the task list at the top of this file and the
two Records named under "How the work is taken"; §13 Proximity, §9 Federation
and Blood and the Recipe origin stamp remain PARKED — directions rather than
scheduled work. The Karma mechanism's prose lives in `Karma.md` §14.1 and its
boxes live here, because that file is the specification and this list is the
plan.

Three boxes sit under Retention that are NOT log integrity — they are
per-contact configuration and scheduled work:
...
The boxes under "Seven architectural decisions" are not work of their own; each
belongs to whatever its decision serves — epochs and succession to reachability
and identity, snapshots and storage to the log, Cell capabilities to the
Organ/Cell split, the drift constant and the test harness to bedrock.
...
ask for `head` and not `body`. The one selector the whole thing hangs off had
...
over tables, matching its own name-what-you-want rule — a sixth
...
prose cannot outlive the mechanism it described.
  What remains of this entry is its hazard, not its mechanism: a Record entering
  is a grant and one leaving is a revoke, both fired by a change that nobody
  meant as an act of sharing. The guardrail is that a rule answers "what are you
  sharing right now" BEFORE it can be switched on — a read off the membership
  table, not new work.
...
**Revoke is local and that is the whole feature — decided 2026-08-18.**
Revoking stops every future op for that Record: hard, local, guaranteed,
nothing more travels. The remote still holds what it already saw, and no
instruction can change that — their Cell runs their code and promised us
nothing.

**A `forget` op was specified here and is DROPPED.** The idea was a polite
"please delete your copy" that the remote might honour, with honouring read as a
trust signal. It buys nothing a message does not: if you want somebody to delete
something, you ask them, in the conversation you already have with them. Making
it a protocol feature would cost a wire change and would put a button in the
interface whose honest label is "sent a request that may be ignored" — which is
what a sentence in a chat already is, without the machinery.

What survives is the rule about language: never let the interface imply revoke
reaches into another Organ. "Stopped sharing" is true. "Deleted from their
device" never is.
...
## 13. Proximity is a bond, and physical nearness is not it (post-sync)

`organ_contact.proximity` is a numeric, LOCAL-ONLY score of how close a
contact is to you as a RELATIONSHIP — family, an old friend, someone you met
once. **You set it**, through `SetContactProximity`; nothing derives it, and
it never travels outward (§2). Nothing in sync depends on it.

That is the whole of what `proximity` means. The rest of this section is about
PHYSICAL nearness, which is a different quantity that shared the name here for
a long time and must not share the column: if it is ever built it gets its own,
called `nearness`. Letting a radio write to `proximity` would overwrite a
person's own statement about a relationship with a fact about which router they
are on — the neighbour promoted to close friend, the old friend abroad decayed
to a stranger.

This part is future work, layered after sync ships; it exists so the signals
below land in one place instead of leaking into discovery (§11), which only
*finds* peers and never scores nearness.
...
2. **BLE — Bluetooth Low Energy.** Radios take two roles: *advertising*
   ...
   can scan but advertise poorly. So BLE nearness is mobile-first by
...
## The release train, when there is anyone to plan around

**Deferred to here 2026-08-19.** Wire changes were to be held and released as
one numbered epoch per release train, so that a hard cut of old peers happens
on a schedule people can plan around rather than whenever a cleanup lands. The
protective half of that decision is already built and stays built:
`lince/hello/1`, the one ALPN that must never be bumped, is what lets a device
one release behind report *this device needs updating* instead of being
indistinguishable from a device that is switched off.

What is deferred is the scheduling — batching queued wire changes behind a
numbered train instead of breaking whenever the change is ready. It buys
nothing yet: nobody is running Lince, so there is no stale peer to strand and
no app store review to wait out, and a cut costs one rebuild. It becomes real
...
"**This section is [[Resenha|r_NPSFP32HZJK064WP93XH0HKJZJ]]**" (2026-08-18), and
its boxes live there with the five deployments that were waiting on them. The build order is the node
list and normal mode, then the simulated environment (clock, network and disk
behind traits the engine already goes through — the load-bearing item, and
every direct `Utc::now()` in the engine is a hole in it), then scripted mode
with the deferred suite as its first scenarios, then random mode, then fault
injection worth the name.

A partition that is only "both nodes paused" tests nothing. The asymmetric
cases — one-way partition, delay, reorder, duplicate, and a Cell that stops and
comes back with an old clock — are where every one of this document's sync
decisions actually lives, which is why fault injection is listed as a build
item rather than assumed to come free with the simulator.
...
Nothing below is scheduled. It is the pool to draw from: move a task up into
"What we work on next" when it becomes worth doing.


## Big themes, whole

- [ ] Karma Firing Control per Cell:
  - [ ] Control per cell what rules can get run there.
  - [ ] Make sure rule firing is possible to be infinite (triggering some change in some triggers a rule firing in other) or impossible to happen with a karma/organ policy.
  - [ ] Be able to understand what rules have fired in what cell and what can fire in each cell, when looking at rules.
  Also carried here: **Scheduling across Cells, Karma included** — the two axes, the lease, rule provenance, the generation counter and per-rule budget, single-executor defaults for externally-observable consequences; and the **Surface**: which Cell holds the lease right now, and what fired where.
  AI comments below:
    **Anicca source rule, decided 2026-08-19.** A valid active Karma declaration in an enabled sync directory executes on the Cell watching that directory. The file is the activation surface; there is no second hidden enable step. The Organ panel says this before enabling the folder and shows that Cell's source-backed Rules, Frequencies, quantities, next dates, and conflicts. This task still owns the multi-Cell consequence: designation or leases for outward effects, provenance showing which Cell fired, bounded reaction chains, and an explicit policy that can make cycles impossible or deliberately infinite.
- [ ] **Profile and discovery.** The three tiers, the security defaults,
  the Facade, gossip with seen-set and budgets, hop rules, the directory Cell.
  The largest one left; it needs the identity floor under it, and somewhere
  always-on to run.
  **Surface:** a profile editor per tier that shows what each tier reveals, and
  to whom, BEFORE anything is published. Its whole risk is people publishing more than they meant to.
...
- [ ] **The Facade cache needs a PER-SOURCE quota, not only a global cap** —
  **C9's box now** (moved 2026-08-18, the one item C2c could not close). A
  global cap alone means one hostile publisher fills the cache and evicts
  everyone else's pages at no cost to itself.
  **The mechanism exists and is already in use; only the consumer is missing.**
  `store::budget::evict_plan` is the per-source byte quota as a pure function —
  newest survives, an entry larger than the whole quota is dropped rather than
  permanently occupying an area budgeted for many, and a corrupt length evicts
  instead of wrapping into apparent free space. Quarantine calls it today, per
  contact, alongside its existing per-contact row count: the two bounds answer
  different questions (how much disk one peer may cost me, versus how much
  garbage from one peer is worth reading) and the tighter one wins. When C9
  builds the cache it calls the same function. Writing a second eviction policy
  is how the two areas end up disagreeing, and the difference would only ever be
  found as a bug.

## Discovery, gossip and the directory
...
- [ ] **Hop count is the only distance a Cell without internet or GPS can
  ...
  willing Cells forwarded this." Proximity (§13) is a different thing again —
  the local-only score of how close a contact is to you as a BOND, which you
  set yourself and which nothing computes. Hop count is a network-topology
  fact, not a nearness claim and certainly not a closeness one; none of the
  three may be conflated in the UI.
...
- [ ] **A harness that spawns real processes with separate stores**, and the
  ...
  multi-Cell correctness has any evidence behind it, and everything below
  that touches identity is unverifiable rather than merely untested.

## Nearness — NOT `proximity`

**Read this before any box below.** `organ_contact.proximity` is how close
this person is to you as a BOND — a number you set yourself through
`SetContactProximity`, meaning family, old friend, someone you met once.
Nothing computes it and nothing may. It is not distance, and no radio, network
or sensor has any business writing to it.

Everything in this section is about PHYSICAL nearness, which is a different
quantity that happens to have been given the same name here for a long time.
The two were conflated in this plan until 2026-08-22, and the conflation is
the actual hazard: a LAN sighting that "bumps proximity" would silently
overwrite a person's own statement about a relationship with a fact about
which router they are on. Somebody who moved in next door would be promoted to
close friend; an old friend abroad would decay to a stranger.

So if this is ever built it takes its OWN column and its own name — `nearness`
— and the boxes below are written that way. They stay parked either way: the
"owner's position" (2026-08-09, §13) is that Lince should not tell people who is
physically close.

- [ ] Same-LAN sighting raises NEARNESS, never `proximity`: a verified
  discovery announce from a known contact raises the nearness score with
  time-decay (nearness fades if never seen again); the bump uses the
  *verified* fingerprint, never the announce alone.
- [ ] BLE beacon (mobile): advertise the same transport-agnostic payload
  ...
  contacts and raise NEARNESS by RSSI bucket. Unknown fingerprints are
  ignored — BLE never drives introduction, because there is no channel for
  the challenge handshake until a network exists.
- [ ] Decay + buckets, not meters: nearness is a slowly-decaying score
  ...
  cannot give. The bond score has no decay at all — a friendship does not
  fade because nobody walked past a router.
...
- [ ] **The public face is an Organ you share a subset with**, and its
  narrowing is the outbound per-field scoping above, which does not exist yet
  (inbound acceptance and per-record hiding do). Filed here because the
  DECISION belongs with the infrastructure it shapes; the mechanism it waits
  on is the scoping work and the Facade it serves is the discovery work. The
  mailbox it sat beside had no such dependency and proceeded alone.
  **Surface:** whoever runs it must be able to see what the public Organ
  actually holds — a published-subset view, not a promise.
- [ ] **A published-subset view**: what the public Organ actually holds, shown
  as data rather than promised in a settings screen. The same honest-empty-state
  rule as everywhere else, applied where the consequence of getting it wrong is
  a disclosure rather than an awkwardness.
- [ ] **The Facade is generated by the PUBLIC Organ, not by the internal one**
  (decided 2026-08-15). Not a deployment preference: a generator running where
  everything is visible is trusted to omit the right things, while one running
  on a box that only ever received the published subset cannot leak what it does
  not have. This is why the public face is a separate Organ rather than a Cell.

## Records, Lingua and Instinct

- [ ] **A mention in a body becomes a link.** `[[Title|uid]]` inside body text,
  the first mention of each other Record per file, later mentions left plain.
  Two rules, both about the op log: the **uid is authoritative and the title is
  decoration**, and renaming a Record must NOT rewrite the bodies that mention
  it — a rename that rewrote bodies would be an op storm across every peer for
  a cosmetic change. "Once per file, first mention" is a rule for whoever
  generates the text, never for the parser.
- [ ] **Instinct ships Records, not pages**, with an explicit "put these in my
  Lince" that imports the bundle as a Trail. Importing is a normal consented
  import, so the same view says what it is about to add before it adds it. The
  chapters keep working as reading material either way; what changes is that
  the reader can take the subject matter into their own store, which is what
  lets a later Karma chapter populate Karma and let them watch it run.
...
A relay and a townsquare are different things and must stay separate in
configuration and in the interface, so an operator never believes moderating a
townsquare gives them power over what a relay forwards, or that running a relay
makes them answerable for what a townsquare publishes: stateless plumbing anyone
may run and nobody trusts, versus a social space with a person answerable for
it. There is no middle option — a box that holds plaintext "just to help" is a
full Cell with none of a full Cell's accountability. The choice is
readable-and-yours, or unreadable-and-anyone's.

The first box below is the new role. The other four are shaped by it and cannot
land before it.

- [ ] **Relay Cells, not just `iroh-relay`.** `iroh-relay` only rendezvouses
  and falls back for two Cells that already know each other's NodeId. A
  discovery relay is a different role: a Cell willing to carry OTHER Organs'
  discovery/gossip traffic onward over the internet. New infrastructure, not a
  mode switch on the existing relay.
- [ ] **Consent to relay is explicit per Cell**, the same way running
  `iroh-relay` on the VPS is a deliberate act rather than ambient behaviour
  every Cell does by default — a Cell opts in to spending its own
  bandwidth and battery carrying a stranger's discovery traffic. Tests: a
  Cell that never opted in forwards nothing.
- [ ] **Relay mode uses `relay_capabilities()`.** The empty set exists and is
  ...
  relay work in C4. Tests: a published relay Cell is refused the capabilities
  outside that set.
- [ ] **A relayed batch carries a SIGNED BATCH envelope** from the original
  author, verified on arrival and passed along intact — one signature per
  batch, never per op. Per-op signing is the version that hurts: a burst of
  ...
  as a tax on ordinary sync. Tests: a tampered relayed batch is refused; an
  intact one verifies after two hops.
- [ ] **Relaying a contact's ops is a per-contact setting defaulting to the
  quiet way.** Forwarding a contact's ops to your other contacts is a
  disclosure nobody in that chain consented to, independent of whether it is
  cryptographically sound. Tests: the default forwards nothing.

- [ ] **A relay over long-range low-bandwidth radio.** For the case with no
  internet at all, a relay carrying discovery and gossip onward over LoRa or
  similar. A second transport, not a setting on the internet relay, so it is
  its own work rather than a clause inside it.

## Collab, deferred

- [ ] **`Undelete` and `Restore`.** Deletion today is a hard tombstone with no
  way back, so an undelete must land as a newer lifecycle op before edits
  resume on a frozen doc. The lifecycle action does not exist yet.
