...
pub struct AdminBootstrap {
    ...
    pub mandatory: bool,
    pub staged_password: Option<String>,
}

pub async fn ensure_admin(store: &Store, options: AdminBootstrap) -> Result<(), io::Error> {
    ...
    let (username, password) = if let Some(password) = staged {
        ...
        status(
            "Auth is required but the new store has no admin yet. \
             Start Lince once in an interactive terminal to create the initial admin.",
        );
        return Ok(());
    } else {
