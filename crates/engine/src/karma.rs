        ...
        for row in rows {
            ...
            for key in def.condition.tokens() {
                let record_uid = store::records::resolve(&store.pool, &key.slug)
                    .await?
                    .map(|r| r.uid);
                if record_uid.is_none() && key.func != "promise_state" && key.func != "confidence" {
                    registry.proof_warnings.push(ProofWarning {
                        rules: vec![row.record_uid.clone()],
            ...
            registry.rules.push(LoadedRule { def, tokens, writes, active: row.active });
            let _ = idx;
        }
            ...
            for i in 0..n {
                for &j in &value_deps[i].clone() {
                    let extra: Vec<String> =
                        reads[j].difference(&reads[i]).cloned().collect();
                    if !extra.is_empty() {
                        stable = false;
    ...
    fn proof(&mut self, _scratch: &mut Vec<String>) {
        ...
        for comp in nucleus::graph::cycles(&nodes, &edges) {
            ...
            self.proof_warnings.push(ProofWarning {
                message: format!("these {} rules form a loop: {}", named.len(), named.join(" -> ")),
                rules: comp,
            });
        }
    }
    ...
    for t in &rule.tokens {
        ...
        let value: f64 = match key.func.as_str() {
            "quantity" | "signal" => match &t.record_uid {
                Some(uid) => store::records::quantity(&store.pool, uid).await?.unwrap_or(0.0),
            ...
            "sum" => {
                ...
                let window = key.dur_secs.ok_or_else(|| {
                    EngineError::Consequence(format!(
                        "sum(@{}) needs a duration literal (e.g. 30d)",
                        key.slug
                    ))
                })?;
                store::facts::sum_window(&store.pool, uid, window, now).await?
            }
            "value" => {
            ...
            "hours_since_fact" => {
                let uid = t
                    ...
                    .as_ref()
                    .ok_or_else(|| EngineError::UnknownRecord(key.slug.clone()))?;
                store::facts::hours_since_last(&store.pool, uid, now).await?.unwrap_or(1.0e9)
            }
            ...
            "projected" => {
                ...
                let timeline = nucleus::imagination::project(
                    &snapshot,
                    now + chrono::TimeDelta::seconds(dur),
                );
            ...
            // Pending: route_eta needs loaded map data; demand needs curve queries.
            "route_eta" | "demand" => {
                return Err(EngineError::Consequence(format!(
                    "{}() is specified (blueprint IX/XII) but not implemented yet",
                    key.func
                )));
            }
            other => {
                return Err(EngineError::Consequence(format!("unknown condition function {other}()")));
            }
        };
...
async fn derived_value(
    ...
    let result = async {
        ...
        let mut resolver =
            Box::pin(prefetch(store, registry, rule, injected, now, depth + 1, visiting)).await?;
...
pub async fn cascade(
    ...
    while let Some(record_uid) = pending.pop_front() {
        let Some(reader_idxs) = registry.readers.get(&record_uid) else { continue };
        for &idx in reader_idxs {
            ...
            let rule = &registry.rules[idx];
            if !rule.active || rule.def.is_derived_value() {
                continue;
            }
    ...
    for c in &rule.def.consequences {
        match c.kind {
            ...
            | ConsequenceKind::Deactivate => {
                ...
                let current =
                    store::records::quantity(&store.pool, &target).await?.unwrap_or(0.0);
            ...
            ConsequenceKind::EmitPromise => {
                ...
                let delta = params.get("delta").and_then(|d| d.as_f64()).unwrap_or(carried);
            ...
            ConsequenceKind::Ask => {
                ...
                store::misc::create_decision(&store.pool, &rule.def.uid, "ask", &question, &options)
                    .await?;
            }
            ...
            ConsequenceKind::SetVisibility => {
                ...
                let params = c.params.clone().unwrap_or(serde_json::json!({}));
                let subject_kind =
                    params.get("subject_kind").and_then(|s| s.as_str()).unwrap_or("public");
                let subject = params.get("subject").and_then(|s| s.as_str());
                store::visibility::grant(&store.pool, subject_kind, subject, &target).await?;
            }
            // Specified (VII.2) — land with the transport stage.
            ConsequenceKind::RunQuery | ConsequenceKind::RunAction => {
                return Err(EngineError::Consequence(format!(
                    "{} is specified but not implemented yet (later stage)",
                    c.kind.as_str()
                )));
            }
        }
...
fn display_name(rule: &LoadedRule) -> String {
    rule.def.slug.clone().unwrap_or_else(|| rule.def.uid.clone())
}
