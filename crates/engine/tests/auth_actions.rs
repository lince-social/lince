...
async fn privileged_actor(e: &Engine, permission: &str) -> i64 {
    let role_id = store::auth::ensure_role(&e.store.pool, "wields-it").await.unwrap();
...
async fn each_auth_action_denies_an_actor_without_its_permission() {
    let e = engine().await;
    let bystander = store::auth::ensure_role(&e.store.pool, "bystander-role").await.unwrap();
    let bystander = store::auth::create_user(&e.store.pool, "Bystander", "bystander", "hash", bystander)
        .await
        .unwrap();
    store::auth::ensure_role(&e.store.pool, "existing").await.unwrap();

    let err = e
        .act(Action::CreateRole { name: "new-role".into() }, Some(bystander.to_string()))
        .await
    ...
    let err = e
        .act(
            Action::AssignRole { user: bystander.to_string(), role: "existing".into() },
            Some(bystander.to_string()),
        )
        .await
    ...
    let err = e
        .act(
            Action::GrantPermission { role: "existing".into(), permission: "record:read".into() },
            Some(bystander.to_string()),
        )
        .await
    ...
    let err = e
        .act(
            Action::RevokePermission { role: "existing".into(), permission: "record:read".into() },
            Some(bystander.to_string()),
        )
        .await
        .expect_err("no permission:assign grant");
...
async fn create_role_then_create_user_wires_a_working_login() {
    ...
    let role_id = store::auth::ensure_role(&e.store.pool, "user-creator").await.unwrap();
    let perm_id = store::auth::ensure_permission(&e.store.pool, "user", "create").await.unwrap();
    store::auth::grant(&e.store.pool, role_id, perm_id).await.unwrap();
    ...
    let outcome = e
        .act(Action::CreateRole { name: "support".into() }, Some(admin.to_string()))
        .await
...
async fn assign_role_moves_a_user_between_roles() {
    let e = engine().await;
    let from_role = store::auth::ensure_role(&e.store.pool, "from-role").await.unwrap();
    let to_role = store::auth::ensure_role(&e.store.pool, "to-role").await.unwrap();
    let perm_id = store::auth::ensure_permission(&e.store.pool, "record", "read").await.unwrap();
    store::auth::grant(&e.store.pool, to_role, perm_id).await.unwrap();
    let user_id = store::auth::create_user(&e.store.pool, "Amy", "amy", "hash", from_role)
        .await
        .unwrap();

    e.act(
        Action::AssignRole { user: user_id.to_string(), role: "to-role".into() },
        None,
    )
    .await
    .expect("local mode assigns roles freely");

    let user = store::auth::user_by_id(&e.store.pool, user_id).await.unwrap().unwrap();
...
async fn grant_and_revoke_permission_round_trip() {
    let e = engine().await;
    store::auth::ensure_role(&e.store.pool, "toggled").await.unwrap();

    e.act(
        Action::GrantPermission { role: "toggled".into(), permission: "record:delete_own".into() },
        None,
    )
    .await
    .expect("local mode grants freely");
    let keys = store::auth::role_permission_keys(&e.store.pool, "toggled").await.unwrap();
    assert!(keys.iter().any(|k| k == "record:delete_own"));

    e.act(
        Action::RevokePermission { role: "toggled".into(), permission: "record:delete_own".into() },
        None,
    )
    .await
