...
/// The folder Lince's own documentation lives in, adopted for real.
///
/// Losslessness and adoptability are DIFFERENT failures: `docs/records/` can be
/// a perfectly faithful transcription of the Instinct chapters and still be
/// refused in full by a tick, because one file names a predicate that is not a
/// Concept or links to a uid nobody claims. Nothing else in this suite would
/// notice — every other test writes its own two-file fixture.
#[tokio::test]
async fn the_shipped_documentation_folder_is_adoptable_as_records() {
    let (e, organ) = cell_with_local_organ().await;
    use_lingua(&e, &organ).await;
    // The frozen predicate list, taken from the one place that defines it
    // rather than copied — a copy here would drift the day the folder grows a
    // predicate, and the failure would look like the folder being broken.
    for name in engine::instinct::VOCABULARY {
        concept(&e, name).await;
    }
    ...
    let source = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../docs/records")
        .canonicalize()
        .expect("docs/records is in the repository");
    let dir = tmp_dir();
    let mut expected = 0;
    for entry in std::fs::read_dir(&source).unwrap() {
        let path = entry.unwrap().path();
        if path.extension().and_then(|e| e.to_str()) != Some("lingua") {
            continue;
        }
        std::fs::copy(&path, dir.join(path.file_name().unwrap())).unwrap();
        expected += 1;
    }
    assert!(expected > 20, "the folder has content: {expected}");
