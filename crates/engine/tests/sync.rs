...
async fn a_signed_package_crosses_cells_and_stays_verifiable() {
    ...
    let ana = cell().await;
    ana.set_signer(Signer::generate("ana", "ed25519:ana:2026-07")).await.unwrap();
    ...
    let package = ana.export_package("organ.family", "organ.ana").await.unwrap();
    ...
    assert_eq!(store::records::quantity(&bruno.store.pool, &apples).await.unwrap(), Some(10.0));
    // authorship survived replication: the facts still name Ana
    assert!(applied.iter().all(|f| f.actor_uid.as_deref() == Some("ana")));
...
async fn export_respects_the_visibility_gate() {
    ...
    assert_eq!(package.records.len(), 1, "private record never leaves the Cell");
