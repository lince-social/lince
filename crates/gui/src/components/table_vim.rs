...
use crate::keybinding_mode::Mode;
...
pub fn table_command(event: &KeyDownEvent, mode: Mode, edit_mode: EditMode) -> TableVimCommand {
    match mode {
        Mode::Vim => table_vim_command(event, edit_mode),
        Mode::Normal => normal_text_command(event),
    }
}
...
fn normal_text_command(event: &KeyDownEvent) -> TableVimCommand {
    ...
    match event.keystroke.key.as_str() {
        ...
        "escape" => return TableVimCommand::CancelEdit,
