...
pub(crate) struct SavedFrequencyCastle {
    pub workspace: u64,
...
pub(crate) fn snapshot(world: &mut World, root: Entity) -> Vec<SavedFrequencyCastle> {
    world
        .query::<(
            Entity,
            &ChildOf,
        ...
        .map(|(entity, _, member, item, castle)| SavedFrequencyCastle {
            workspace: member.0,
            castle: castle.clone(),
