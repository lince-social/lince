...
pub(crate) struct SavedKarmaCastle {
    pub workspace: u64,
    castle: KarmaCastle,
    position: [f64; 2],
    size: [f32; 2],
...
pub(crate) fn snapshot(world: &mut World, root: Entity) -> Vec<SavedKarmaCastle> {
    world
        .query::<(
            Entity,
            &ChildOf,
        ...
        .map(|(entity, _, member, item, castle)| SavedKarmaCastle {
            workspace: member.0,
            castle: castle.clone(),
