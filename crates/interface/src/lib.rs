    ...
    pub fn ownership_preflight(host: HostFacts) -> Self {
        ...
        Self {
            ...
            limitations: vec![
                ...
                "CEF accelerated paint callbacks compile without the upstream WGPU helper; the host importer is not integrated yet.".into(),
...
fn dependency_facts() -> Vec<DependencyFact> {
    vec![
        ...
        DependencyFact {
            name: "cef-rs".into(),
            revision: "cef 151.8.0+151.3.24 source profile".into(),
            role: "accelerated external HTML producer".into(),
            license: "MIT OR Apache-2.0; Chromium/CEF notices pending inventory".into(),
            state: "source graph compiled; accelerated surface not integrated".into(),
            gpu_family: Some("host wgpu 29.0.4; upstream wgpu 30 helper excluded".into()),
            raw_window_handle_family: None,
        },
        DependencyFact {
...
fn adapter_facts() -> Vec<AdapterFact> {
    vec![
        ...
        AdapterFact {
            name: "CEF HTML".into(),
            state: "not integrated".into(),
            owner: "isolated Chromium producer through Lince adapter".into(),
            detail: "accelerated surface and authority fixture not exercised".into(),
        },
    ]
}
...
mod tests {
    ...
    fn panel_is_honest_about_unintegrated_adapters() {
        ...
        assert!(panel.contains("CEF HTML           CALLBACK TYPES COMPILED; HOST IMPORT PENDING"));
