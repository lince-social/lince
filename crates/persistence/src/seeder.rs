use crate::repositories::view::sync_all_view_dependencies_in_pool;
...
pub async fn seed(db: &Pool<Sqlite>) -> Result<(), Error> {
    ...
    sqlx::query(
        "INSERT INTO view(name, query)
         SELECT 'Record Creation', 'create_view_record'
         WHERE NOT EXISTS (
            SELECT 1 FROM view WHERE query = 'create_view_record'
         )",
    )
    .execute(&*db)
    .await
    ...
    sqlx::query(
        "INSERT INTO collection_view(quantity, collection_id, view_id)
         SELECT 0, c.id, v.id
         FROM collection c
         JOIN view v ON v.query = 'create_view_record'
         WHERE NOT EXISTS (
            SELECT 1
            FROM collection_view cv
            WHERE cv.collection_id = c.id
    ...
    sync_all_view_dependencies_in_pool(db).await?;
