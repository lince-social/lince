...
/* No max-width/max-height here on purpose. This element's `100vw`/`100vh` is
   the iframe, and the iframe is sized FROM a measurement of this element — a
   cap in viewport units is self-referential: the popover clamps, we report the
   clamped size, the frame shrinks to it, the cap tightens. It can never settle
   on the natural size, and the last row of buttons hangs out past the bottom.
   The board does the real viewport clamp (canvasRect, in syncCardNode), which
   is the side that actually knows how big the canvas is. */
.editPopover{position:fixed;top:0;right:0;bottom:auto;width:fit-content;min-width:0;max-width:none;height:fit-content;max-height:none;gap:0;padding:var(--space-2,4px);overflow:visible;border:var(--hairline,.5px) solid var(--gray);background:var(--raised-background);box-shadow:var(--lynx-shadow-x,2px) var(--lynx-shadow-y,2px) var(--lynx-shadow-blur,6px) var(--lynx-shadow-color,rgb(0 0 0 / 16%));backdrop-filter:none}
...
// getBoundingClientRect, not scrollWidth/scrollHeight: scroll* is the padding
// box rounded to integers, so it drops the border and (on an overflowing box)
// the bottom padding — a handful of pixels, which is exactly enough to leave
// the last row of icons half outside the frame we ask for.
function reportPopoverSize(){if(!popover?.classList.contains("isOpen"))return;const box=popover.getBoundingClientRect();const width=Math.ceil(box.width);const height=Math.ceil(box.height);window.parent.postMessage({type:"lince:widget-content-size",instanceId:window.frameElement?.dataset?.packageInstanceId||"",payload:{width,height}},"*");}new ResizeObserver(reportPopoverSize).observe(popover);
