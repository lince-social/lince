...
In simpler terms: you set two different Karma, one checks if 'Read Book' has quantity 0, if yes, make it 1.
The other Karma opens the book if the quantity of 'Read Book' is zero. So when we set it through zero it will open the book once, for
we have a rule that quickly moves it to not zero.
