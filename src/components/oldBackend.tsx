...
//     max_quantity_config = configuration_df[
//         configuration_df["quantity"] == configuration_df["quantity"].max()
//     ].iloc[0]
...
//             case "view_id":
//                 info += (
//                     "Responsible for referencing the view that goes into configuration."
//                 )
//             case "column_information_mode":
...
//         "select truncation, table_query from configuration order by quantity DESC limit 1"
//     ).iloc[0]
//
...
//                 rows[column] = rows[column].apply(
//                     lambda x: truncate_column(x, truncation_size)
//                 )
//
//     return rows
//
...
//     if set_clause != "":
//         command = f"UPDATE {table} SET {set_clause}"
//
//     if where_id_in != None:
//         command += where_id_in
//
...
//         f"UPDATE configuration SET quantity = CASE WHEN id = {id} THEN 1 ELSE 0 END"
//     )
//
//
...
//         os.path.join(
//             __file__, "..", "..", "..", input("File path starting from the lince dir: ")
//         )
//     )
//
...
//         execute_sql_command(
//             command=f"UPDATE record SET quantity = 0 WHERE ID = {operation[0]}"
//         )
//         return dump_db()
//
//     where_id_in = None
...
//                 return restore_db()
//             case "c" | "C":
//                 return create_row(table)
//             case "r" | "R":
//                 return read_rows(f"SELECT * FROM {table}", where_id_in, view_mode=True)
//             case "u" | "U":
//                 update_rows(
//                     table, set_clause=None, where_clause=None, where_id_in=where_id_in
//                 )
//                 return karma()
//             case "d" | "D":
//                 return delete_rows(table, where_clause=None, where_id_in=where_id_in)
//             case "f" | "F":
//                 execute_sql_command_from_file()
//                 return karma()
//             case "a" | "A":
//                 return activate_configuration(operation[1])
//             case "q" | "Q":
//                 execute_sql_command(command=input("Type the SQL command: "))
//                 return karma()
//
//     return True
//
