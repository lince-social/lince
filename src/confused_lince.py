...
def execute_query(query):
    ...
    if query.startswith("SELECT"):
        return pd.Dataframe(conn.cursor().fetchall(), columns=[desc[0] for desc in conn.cursor().description])
...
def generate_db_version(base_filename):
    counter = 0
...
#     base_filename = 'lincedb'
    #     unique_filename = generate_db_version(base_filename)
