...
pub fn socket_v4(interface_addr: Option<Ipv4Addr>) -> Result<UdpSocket, SocketError> {
    ...
    // Do not set SO_REUSEPORT here. On Linux it forms a reuse-port group with
    // other mDNS users (such as browsers) and can distribute UDP datagrams to
    // just one member. mDNS must instead be delivered to every process that
    // joined the multicast group; SO_REUSEADDR is sufficient for that.
    socket
        .bind(&bind_addr)
...
pub fn socket_v6() -> Result<UdpSocket, SocketError> {
    ...
    // See the IPv4 path: SO_REUSEPORT can make mDNS datagrams compete between
    // local consumers, while SO_REUSEADDR retains multicast delivery.
    socket
